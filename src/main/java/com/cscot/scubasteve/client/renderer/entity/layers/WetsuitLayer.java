package com.cscot.scubasteve.client.renderer.entity.layers;
...
// This class currently is not utilized future expansion may require it if Dyeable Armor can be figured out

@OnlyIn(Dist.CLIENT)
public class WetsuitLayer<T extends LivingEntity, M extends BipedModel<T>, A extends BipedModel<T>> extends LayerRenderer<T, M> {
    ...
    protected WetsuitLayer(IEntityRenderer<T, M> p_i50951_1_, A p_i50951_2_, A p_i50951_3_) {
        super(p_i50951_1_);
    ...
    public void render(T entityIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
        this.renderArmorLayer(entityIn, limbSwing, limbSwingAmount, partialTicks, ageInTicks, netHeadYaw, headPitch, scale, EquipmentSlotType.CHEST);
        this.renderArmorLayer(entityIn, limbSwing, limbSwingAmount, partialTicks, ageInTicks, netHeadYaw, headPitch, scale, EquipmentSlotType.LEGS);
        this.renderArmorLayer(entityIn, limbSwing, limbSwingAmount, partialTicks, ageInTicks, netHeadYaw, headPitch, scale, EquipmentSlotType.FEET);
        this.renderArmorLayer(entityIn, limbSwing, limbSwingAmount, partialTicks, ageInTicks, netHeadYaw, headPitch, scale, EquipmentSlotType.HEAD);
    }
        ...
        if (itemstack.getItem() instanceof ArmorItem) {
            ...
            if (armoritem.getEquipmentSlot() == slotIn) {
                ...
                if (armoritem instanceof net.minecraft.item.IDyeableArmorItem) { // Allow this for anything, not only cloth
                    ...
                    this.bindTexture(this.getArmorResource(armoritem, flag, "overlay"));
                }
